...
void main()
{
	...
	vertex_data.position = vec4(position,1.0);
	gl_Position = u_mvp * vec4(position, 1.0);

	vertex_data.uv = a_uv;
	vertex_data.normal = (u_world * vec4(a_normal, 0.0)).xyz;
	vertex_data.world_position = (u_world * vec4(position,1.0)).xyz;
	vertex_data.light_space_pos = u_light_mvp * vec4(position, 1.0);
}
