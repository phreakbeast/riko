...
{
	GuiFrame* frame = (GuiFrame*)malloc(sizeof(GuiFrame));
	assert(frame);
...
void gui_frame_add_widget(GuiFrame* frame, GuiWidget* widget)
{
	...
	{
		frame->num_widgets = 1;
		frame->widgets = (GuiWidget**)(malloc(sizeof(GuiWidget*) * frame->num_widgets));
	...
	{
		frame->num_widgets++;
		frame->widgets = (GuiWidget**)realloc(frame->widgets, sizeof(GuiWidget*) * frame->num_widgets);
		assert(frame->widgets);
