...
bool image_load_bmp(Image *img, const char *filename)
{
    ...
    bmp.pixels = (u8*)malloc(sizeof(u8) * bmp.num_pixels);
    ...
    else
    {
        ...
        u8* padding_row = (u8*)malloc(sizeof(u8) * padding_row_size);
    ...
    img->pixels = (u8*)malloc(sizeof(u8) * img->width * img->height * img->internal_format);
