...
void read_tga_24bits(FILE *fp, TgaImage *img, bool mirrored)
{
    u32 i;
...
void read_tga_32bits(FILE *fp, TgaImage *img, bool mirrored)
{
    u32 i;
    for (i = 0; i < img->width * img->height; ++i)
    {
...
void read_tga_24bits_rle(FILE *fp, TgaImage *img, bool mirrored)
{
    int i, size;
    u8 rgb[3];
    u8 packet_header;
...
void read_tga_32bits_rle(FILE *fp, TgaImage *img, bool mirrored)
{
    int i, size;
    u8 rgba[4];
    u8 packet_header;
...
bool image_load_tga(Image *img, const char *filename)
{
    ...
    image.pixels = (u8*)malloc(sizeof(u8) * image.width * image.height * image.internal_format);

    //5th bit of the image descriptor indicates the origin point
    //0 = Origin in lower left-hand corner.
    //1 = Origin in upper left-hand corner.
    bool mirrored = (header.image_descriptor & (1 << 5)) >> 5;
    ...
    switch (header.image_type) {
        ...
        case 2: //16-24-32
            switch (header.pixel_depth) {
                ...
                case 24:
                    //printf("tga 24bits\n");
                    read_tga_24bits(file, &image, mirrored);
                    break;

                case 32:
                    //printf("tga 32bits\n");
                    read_tga_32bits(file, &image, mirrored);
                    break;
            }
            break;
        ...
        case 10: //16-24-32bits rle
            switch (header.pixel_depth) {
                ...
                case 24:
                    ...
                    read_tga_24bits_rle(file, &image, mirrored);
                    break;

                case 32:
                    //printf("tga 32bits rle\n");
                    read_tga_32bits_rle(file, &image, mirrored);
                    break;
            }
            break;
    ...
    img->pixels = (u8*)malloc(sizeof(u8) * image.width * image.height * image.internal_format);
    memcpy(img->pixels, image.pixels, sizeof(u8) * image.width * image.height * image.internal_format);

    free(image.pixels);

    //TODO: load mirrored images properly
    if (mirrored)
    {
        image_mirror_vertical(img);
    }
