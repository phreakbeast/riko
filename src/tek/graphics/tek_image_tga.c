...
bool image_load_tga(Image *img, const char *filename)
{
    ...
    image.pixels = malloc(sizeof(u8) * image.width * image.height * image.internal_format);
    ...
    img->pixels = malloc(sizeof(u8) * image.width * image.height * image.internal_format);
