...
bool sb_init(u32 width, u32 height, u64 max_sprites)
{
    ...
    GLuint* indices = (GLuint*)malloc(sizeof(GLuint) * indices_size);
    assert(indices);
...
void sb_flush()
{
    ...
    shader_uniform_mat4(g_shader, "u_mvp", &g_ortho, 1, false);
    
    shader_uniform_int(g_shader, "u_texture_0", 0);
    shader_uniform_int(g_shader, "u_texture_1", 1);
    shader_uniform_int(g_shader, "u_texture_2", 2);
    shader_uniform_int(g_shader, "u_texture_3", 3);
    shader_uniform_int(g_shader, "u_texture_4", 4);
    shader_uniform_int(g_shader, "u_texture_5", 5);
    shader_uniform_int(g_shader, "u_texture_6", 6);
    shader_uniform_int(g_shader, "u_texture_7", 7);
...
void sb_render_circle(Vec2 pos, float radius, float start_angle, float end_angle, u32 num_segments, Color color)
{
    ...
    Vec3* points = (Vec3*)malloc(sizeof(Vec3) * (num_segments + 2));
