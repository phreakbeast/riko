...
{    
    ...
    MeshVertexData* vertices = (MeshVertexData*)malloc(sizeof(MeshVertexData) * num_vertices);
    assert(vertices);

    u32 num_indices = rings * sectors * 6;
    u32* indices = (u32*)malloc(sizeof(u32) * num_indices);
    assert(indices);
...
bool load_obj(const char* filename, MeshVertexData** vertices, u32* num_vertices, u32** indices, u32* num_indices)
{
    ...
    *vertices = (MeshVertexData*)malloc(sizeof(MeshVertexData) * num_faces * 3);
    assert(vertices);
    *indices = (u32*)malloc(sizeof(u32) * num_faces * 3);
    assert(indices);
...
void mesh_buffer_add_vertices(MeshBuffer* buffer, MeshVertexData* vertices, u32 num_vertices, u32* indices, u32 num_indices)
{
    if (buffer->num_vertices == 0 && buffer->num_indices == 0)
    {
        ...
        buffer->res_vertices = (MeshVertexData*)malloc(sizeof(MeshVertexData) * num_vertices);
        assert(buffer->res_vertices);
        buffer->res_indices = (u32*)malloc(sizeof(u32) * num_indices);
    ...
    else
    {
        ...
        buffer->num_vertices += num_vertices;
        buffer->num_indices += num_indices;
        buffer->res_vertices = (MeshVertexData*)realloc(buffer->res_vertices, sizeof(MeshVertexData) * buffer->num_vertices);
        assert(buffer->res_vertices);
        buffer->res_indices = (u32*)realloc(buffer->res_indices, sizeof(u32) * buffer->num_indices);
        assert(buffer->res_indices);
...
{
    PickVertexData* vertices = (PickVertexData*)malloc(sizeof(PickVertexData) * buffer->num_vertices);
    assert(vertices);
