...
static Shader* g_animation_shader;
static Shader* g_shadow_animation_shader;
...
void renderer_destroy()
{
    ...
    shader_destroy(g_shadow_animation_shader);
    free(g_shadow_animation_shader);
    shader_destroy(g_animation_shader);
    free(g_animation_shader);
    sb_destroy();
    sbuffer_destroy();
    fb_destroy(g_fb);
    free(g_fb);
...
{   
    ...
    g_shadow_animation_shader = shader_load("shadow_map_animation.glsl");
    if (!g_shadow_animation_shader)
    {
        return false;
    }

    g_animation_shader = shader_load("animation.glsl");
    if (!g_animation_shader)
    {
        return false;
    }
...
void renderer_render_sprite(Rect dest, Rect src, u32 texture_id, Mat4* ortho)
{
    ...
    shader_uniform_mat4(g_sprite_shader, "u_mvp", ortho, 1, false);
...
void renderer_draw_mesh(Mesh *mesh, Material* material, Transform* transform, Camera *cam, SunLight* light, PointLight* point_lights, u32 num_point_lights, Mat4* light_matrix)
{
    ...
    if (material)
    {
        shader_uniform_vec4(g_geometry_shader, "u_material.ambient_color", color_to_vec4(material->ambient_color));
        shader_uniform_vec4(g_geometry_shader, "u_material.diffuse_color", color_to_vec4(material->diffuse_color));
        shader_uniform_vec4(g_geometry_shader, "u_material.specular_color", color_to_vec4(material->specular_color));
        if (material->diffuse_map)
        {
            shader_uniform_int(g_geometry_shader, "u_material.has_diffuse_map", 1);
            texture_bind(material->diffuse_map, 0);
        }
        else
        {
            shader_uniform_int(g_geometry_shader, "u_material.has_diffuse_map", 0);
        }
        if (material->specular_map)
        {
            shader_uniform_int(g_geometry_shader, "u_material.has_specular_map", 1);
            texture_bind(material->specular_map, 1);
        }
        else
        {
            shader_uniform_int(g_geometry_shader, "u_material.has_specular_map", 0);
        }
        if (material->normal_map)
        {
            shader_uniform_int(g_geometry_shader, "u_material.has_normal_map", 1);
            texture_bind(material->normal_map, 2);
        }
        else
        {
            shader_uniform_int(g_geometry_shader, "u_material.has_normal_map", 0);
        }
        shader_uniform_float(g_geometry_shader, "u_material.specular_level", material->specular_level);
        shader_uniform_float(g_geometry_shader, "u_material.specular_power", material->specular_power);
    }
    ...
    Mat4 light_mvp = mat4_transposed(light_matrix);

    shader_uniform_mat4(g_geometry_shader, "u_mvp", &mvp, 1, false);
    shader_uniform_mat4(g_geometry_shader, "u_world", &world, 1, false);
    shader_uniform_mat4(g_geometry_shader, "u_light_mvp", &light_mvp, 1, false);

    if (light)
    {
        shader_uniform_int(g_geometry_shader, "u_use_light", 1); //use light
        shader_uniform_vec3(g_geometry_shader, "u_direct_light.direction", light->direction);
        shader_uniform_vec3(g_geometry_shader, "u_direct_light.color", light->color);
        shader_uniform_float(g_geometry_shader, "u_direct_light.intensity", light->intensity);
        shader_uniform_vec3(g_geometry_shader, "u_eye_world_pos", cam->position);
    }
    else
    {
        shader_uniform_int(g_geometry_shader, "u_use_light", 0); //use light
    }

    shader_uniform_int(g_geometry_shader, "u_num_point_lights", num_point_lights); //use light
    for (u32 i = 0; i < num_point_lights; ++i)
    {
        ...
        sprintf(name1,"u_point_lights[%d].color",i);
        sprintf(name2,"u_point_lights[%d].ambient_intensity",i);
        sprintf(name3,"u_point_lights[%d].diffuse_intensity",i);
        sprintf(name4,"u_point_lights[%d].position",i);
        sprintf(name5,"u_point_lights[%d].atten.constant",i);
        sprintf(name6,"u_point_lights[%d].atten.linear",i);
        sprintf(name7,"u_point_lights[%d].atten.exp",i);

        PointLight* p = &point_lights[i];
        shader_uniform_vec3(g_geometry_shader, name1, p->color);
        shader_uniform_float(g_geometry_shader, name2, p->ambient_intensity);
        shader_uniform_float(g_geometry_shader, name3, p->diffuse_intensity);
        shader_uniform_vec3(g_geometry_shader, name4, p->position);
        shader_uniform_float(g_geometry_shader, name5, p->atten.constant);
        shader_uniform_float(g_geometry_shader, name6, p->atten.linear);
        shader_uniform_float(g_geometry_shader, name7, p->atten.exp);
    }
...
void renderer_draw_mesh_shadow(Mesh* mesh, Transform* transform, Mat4* mvp)
{
    ...
    shader_uniform_mat4(g_shadow_shader, "u_mvp", &mvp_t, 1, false);
...
void renderer_draw_mesh_instanced(Mesh* mesh, Texture* texture, Transform* transforms, u32 num_transforms, Camera* cam, SunLight* light)
{
    ...
    if(texture)
    {
        ...
        shader_uniform_int(g_geometry_instanced_shader, "u_use_texture", 1);//use texture
    }
    else
    {
        shader_uniform_int(g_geometry_instanced_shader, "u_use_texture", 0);//use texture
    }
    
    Mat4* mvp_mats = (Mat4*)malloc(sizeof(Mat4) * num_transforms);
    Mat4* world_mats = (Mat4*)malloc(sizeof(Mat4) * num_transforms);
    ...
    if(light)
    {
        shader_uniform_int(g_geometry_instanced_shader,"u_use_light",1); //use light
        shader_uniform_vec3(g_geometry_instanced_shader,"u_direct_light.direction",light->direction);
        shader_uniform_vec3(g_geometry_instanced_shader,"u_direct_light.color",light->color);
        shader_uniform_float(g_geometry_instanced_shader,"u_direct_light.intensity",light->intensity);
        shader_uniform_vec3(g_geometry_instanced_shader,"u_eye_world_pos",cam->position);
    }
    else
    {
        shader_uniform_int(g_geometry_instanced_shader, "u_use_light", 0); //use light
    }
...
void renderer_draw_pick_mesh(PickMesh *mesh, Transform* transform, Camera *cam)
{
    ...
    if(transform)
    {
        Mat4 mvp = mat4_mul3(&cam->projection, &cam->view, &transform->matrix);
        mvp = mat4_transposed(&mvp);
        shader_uniform_mat4(g_picking_shader, "u_mvp",&mvp,1,false);
    }
    else
    {
        Mat4 mvp = mat4_mul2(&cam->projection, &cam->view);
...
void renderer_draw_terrain(Terrain* terrain, Camera* cam, SunLight* light)
{
    ...
    shader_uniform_mat4(g_terrain_shader, "u_mvp", &mvp, 1, false);
    shader_uniform_mat4(g_terrain_shader, "u_world", &world, 1, false);
    
    if(light)
    {
        shader_uniform_int(g_terrain_shader,"u_use_light",1); //use light
        shader_uniform_vec3(g_terrain_shader,"u_direct_light.direction",light->direction);
        shader_uniform_vec3(g_terrain_shader,"u_direct_light.color",light->color);
        shader_uniform_float(g_terrain_shader,"u_direct_light.intensity",light->intensity);
        shader_uniform_vec3(g_terrain_shader,"u-eye_world_pos",cam->position);
    }
    else
    {
        shader_uniform_int(g_terrain_shader,"u_use_light", 0); //use light
    }
...
void renderer_draw_shape(Shape* shape, Mat4* transform, Camera* cam)
{
    ...
    shader_uniform_mat4(g_shape_shader, "u_mvp", &mvp, 1, false);
...
void renderer_draw_water(Water* water, Camera *cam)
{
    ...
    shader_uniform_mat4(g_water_shader, "u_mvp", &mvp, 1, false);
...
{
    shader_bind(g_animation_shader);

    if (material)
    {
        shader_uniform_vec4(g_animation_shader, "u_material.ambient_color", color_to_vec4(material->ambient_color));
        shader_uniform_vec4(g_animation_shader, "u_material.diffuse_color", color_to_vec4(material->diffuse_color));
        shader_uniform_vec4(g_animation_shader, "u_material.specular_color", color_to_vec4(material->specular_color));
        if (material->diffuse_map)
        {
            shader_uniform_int(g_animation_shader, "u_material.has_diffuse_map", 1);
            texture_bind(material->diffuse_map, 0);
        }
        else
        {
            shader_uniform_int(g_animation_shader, "u_material.has_diffuse_map", 0);
        }
        if (material->specular_map)
        {
            shader_uniform_int(g_animation_shader, "u_material.has_specular_map", 1);
            texture_bind(material->specular_map, 1);
        }
        else
        {
            shader_uniform_int(g_animation_shader, "u_material.has_specular_map", 0);
        }
        if (material->normal_map)
        {
            shader_uniform_int(g_animation_shader, "u_material.has_normal_map", 1);
            texture_bind(material->normal_map, 2);
        }
        else
        {
            shader_uniform_int(g_animation_shader, "u_material.has_normal_map", 0);
        }
        shader_uniform_float(g_animation_shader, "u_material.specular_level", material->specular_level);
        shader_uniform_float(g_animation_shader, "u_material.specular_power", material->specular_power);
    }
    ...
    shader_uniform_mat4(g_animation_shader, "u_mvp", &mvp, 1, false);
    shader_uniform_mat4(g_animation_shader, "u_world", &world, 1, false);

    u32 frame_num = (u32)frame;
    float frame_t = frame - frame_num;
    shader_uniform_float(g_animation_shader, "u_lerp", frame_t);

    if (light)
    {
        shader_uniform_int(g_animation_shader, "u_use_light", 1); //use light
        shader_uniform_vec3(g_animation_shader, "u_direct_light.direction", light->direction);
        shader_uniform_vec3(g_animation_shader, "u_direct_light.color", light->color);
        shader_uniform_float(g_animation_shader, "u_direct_light.intensity", light->intensity);
        shader_uniform_vec3(g_animation_shader, "u_eye_world_pos", cam->position);
    }
    else
    {
        shader_uniform_int(g_animation_shader, "u_use_light", 0); //use light
    }

    shader_uniform_int(g_animation_shader, "u_num_point_lights", num_point_lights); //use light
    for (u32 i = 0; i < num_point_lights; ++i)
    {
        char name1[128];
        char name2[128];
        ...
        PointLight* p = &point_lights[i];
        shader_uniform_vec3(g_animation_shader, name1, p->color);
        shader_uniform_float(g_animation_shader, name2, p->ambient_intensity);
        shader_uniform_float(g_animation_shader, name3, p->diffuse_intensity);
        shader_uniform_vec3(g_animation_shader, name4, p->position);
        shader_uniform_float(g_animation_shader, name5, p->atten.constant);
        shader_uniform_float(g_animation_shader, name6, p->atten.linear);
        shader_uniform_float(g_animation_shader, name7, p->atten.exp);
    }

    anim_render(anim,frame_num);
    g_drawcalls++;
}

void renderer_draw_animation_shadow(animation_t* anim, Transform* transform, float frame, Mat4* mvp)
{
    shader_bind(g_shadow_animation_shader);

    Mat4 mvp_t = mat4_mul2(mvp, &transform->matrix);
    mvp_t = mat4_transposed(&mvp_t);

    shader_uniform_mat4(g_shadow_animation_shader, "u_mvp", &mvp_t, 1, false);

    u32 frame_num = (u32)frame;
    float frame_t = frame - frame_num;
    shader_uniform_float(g_shadow_animation_shader, "u_lerp", frame_t);
