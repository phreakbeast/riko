...
void noise_init(Noise* noise, u32 width, u32 height)
{
    noise->width = width;
    noise->height = height;
    noise->values = (float*)malloc(sizeof(float) * width * height);
...
static double smooth_noise(Noise* noise, double x, double y)
{
...
static double turbulence(Noise* noise, double x, double y, double size)
{
...
{
    return (float)turbulence(noise, x, y, 16.0);
}
