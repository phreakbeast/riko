#include "tek_perlin.hpp"
#include <stdlib.h>

void perlin1d_init(Perlin1D* perlin, int size, int octaves, float bias)
{
    perlin->output_size = size;
    perlin->seed = (float*)malloc(sizeof(float) * perlin->output_size);
    perlin->noise_values = (float*)malloc(sizeof(float) * perlin->output_size);
...
void perlin2d_init(Perlin2D* perlin, int width, int height, int octaves, float bias)
{
    ...
    perlin->seed = (float*)malloc(sizeof(float) * output_size);
    perlin->noise_values = (float*)malloc(sizeof(float) * output_size);
